...
function linkCard(filters: SearchFilters): Property {
  return {
    ...
    title: `חפש ביד2${filters.city ? ` — ${filters.city}` : ''}`,
    price: null,
    rooms: null,
    floor: null,
    totalFloors: null,
    ...
    advertiserType: null,
    handType: null,
    description: 'לחץ לחיפוש ישיר ביד2 עם הפילטרים שנבחרו',
    images: [],
    publishedAt: '',
...
export async function searchYad2(filters: SearchFilters): Promise<Property[]> {
  try {
    const params: Record<string, string | number> = { mainCategory: 1 };
    ...
    const feed: unknown[] = res.data?.data?.feed?.feed_items ?? [];
    if (feed.length === 0) return [linkCard(filters)];

    return feed
      .filter((item): item is Record<string, unknown> => !!item && typeof item === 'object' && 'id' in item)
      .map((item) => ({
        id: `yad2-${item.id}`,
        ...
        images: (Array.isArray(item.images) ? item.images : []).map((img: { src: string }) => img.src),
